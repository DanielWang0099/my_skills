...
python3 "$HOME/.codex/skills/.system/skill-installer/scripts/install-skill-from-github.py" \
  ...
  --ref main \
  --path skills/c-academic-research-suite \
  --method git
```
...
```bash
rm -rf "$HOME/.codex/skills/c-academic-research-suite"
python3 "$HOME/.codex/skills/.system/skill-installer/scripts/install-skill-from-github.py" \
  --repo Imbad0202/academic-research-skills-codex \
  --ref main \
  --path skills/c-academic-research-suite \
...
```text
Use $c-academic-research-suite to plan a systematic literature review on AI in higher education QA.
```

The skill name is singular: `$c-academic-research-suite`.

Verify the install with `/skills`. You should see one ARS entry only:
`c-academic-research-suite` or `Academic Research ...`. If you see separate
...
`$c-academic-research-suite` router.
...
```text
Use $c-academic-research-suite: ars-outline for this manuscript draft.
```
...
```text
Use $c-academic-research-suite to start academic-pipeline.
...
codex exec --ephemeral --sandbox read-only \
  ...
  'Use $c-academic-research-suite. Router smoke test only. User request to classify: I want to write a paper on AI adoption in higher education quality assurance, but I do not yet have a clear research question. According to the c-academic-research-suite router, classify the workflow and mode.'
```
...
```bash
cd skills/c-academic-research-suite/ars
...
| Native `/ars-*` slash-command registration | Not available. Use `ars-*` aliases through `$c-academic-research-suite`. |
...
```bash
python3 -m json.tool skills/c-academic-research-suite/manifest.json
python3 skills/c-academic-research-suite/ars/scripts/check_data_access_level.py --path skills/c-academic-research-suite/ars
python3 skills/c-academic-research-suite/ars/scripts/check_task_type.py --path skills/c-academic-research-suite/ars
```
