...
def test_vague_paper_topic_routes_to_deep_research_socratic() -> None:
    ...
    plan = planner.plan_request(
        "Use $c-academic-research-suite. I want to write a paper on AI adoption in higher education quality assurance. I do not yet have a clear research question.",
        env={},
    )
...
def test_vague_topic_with_unclear_research_question_still_routes_to_socratic() -> None:
    planner = _load_planner()
    plan = planner.plan_request(
        "Use $c-academic-research-suite. I want to write a paper on AI governance in universities, but my research question is still unclear.",
        env={},
    )
    assert plan["workflow"] == "deep-research"
    assert plan["mode"] == "socratic"
